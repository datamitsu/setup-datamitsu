...
steps:
  ...
  - uses: datamitsu/setup-datamitsu@v0.2.1
```

That installs datamitsu `0.2.1` and runs `datamitsu init` to provision the managed
...
Each release tag pins **one** datamitsu version: `@v0.2.1` installs datamitsu
`0.2.1`. The SHA-256 hashes of every release binary are baked into that tag
(inlined in the non-minified `dist/index.mjs`) and verified on download — an
unverified or missing hash makes the action fail.

Pin by full version (`@v0.2.1`) or commit SHA, **not** a moving major tag, and
